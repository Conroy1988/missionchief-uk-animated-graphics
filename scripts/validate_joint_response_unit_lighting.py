#!/usr/bin/env python3
"""Reject any return of the Joint Response Unit roof-box regression."""
...
def render_before_after(static: Image.Image, release: str) -> Path:
    ...
    draw.text((24, 18), "Joint Response Unit roof-light repair — MissionChief slot 20", font=font(28), fill="white")
    draw.text((340, 64), f"{BASELINE} · oversized dual-service box", font=font(17), fill=(255, 166, 166))
    ...
    target = ROOT / "assets" / "previews" / release / "joint-response-unit-roof-light-before-after.png"
...
def main() -> None:
    ...
    if master.size != (64, 22):
        raise SystemExit(f"Joint Response Unit master dimensions changed: {master.size}")
    if static.size != (72, 30):
        raise SystemExit(f"Joint Response Unit command dimensions changed: {static.size}")
    if old_static.size != (72, 32):
        raise SystemExit(f"Joint Response Unit baseline dimensions changed: {old_static.size}")
    if master.getchannel("A").getbbox() != (0, 1, 64, 22):
        raise SystemExit("Joint Response Unit master no longer has the approved low roofline and bottom anchor")
    if static.getchannel("A").getbbox()[-1] != static.height:
        raise SystemExit("Joint Response Unit command graphic moved away from its bottom-centre map anchor")

    require_isolated_pair(master, (26, 1, 36, 2), {(28, 1), (33, 1)}, "master")
    require_isolated_pair(static, (30, 5, 40, 6), {(32, 5), (37, 5)}, "command static")

    expected_pixels = [(32, 5), (37, 5), (66, 16), (7, 15)]
    actual_pixels = [(int(item["x"]), int(item["y"])) for item in detail["response_light_pixels"]]
    if actual_pixels != expected_pixels:
        raise SystemExit(f"Joint Response Unit animation fixtures changed: expected {expected_pixels}, found {actual_pixels}")
    if len(fixtures["vehicles"].get(ASSET_ID, [])) != 4:
        raise SystemExit("Joint Response Unit must retain two roof emitters plus front and rear emitters")
    ...
    with Image.open(ANIMATED_PATH) as animation:
        if int(getattr(animation, "n_frames", 1)) != 12:
            raise SystemExit("Joint Response Unit APNG must contain 12 full frames")
        for index in range(animation.n_frames):
            animation.seek(index)
            frame = animation.convert("RGBA")
            if index == 0 and ImageChops.difference(frame, static).getbbox() is not None:
                raise SystemExit("Joint Response Unit APNG frame zero no longer matches the static PNG")
            require_isolated_pair(frame, (30, 5, 40, 6), {(32, 5), (37, 5)}, f"frame {index}")
            difference = ImageChops.difference(frame, static)
            for point in ((32, 5), (37, 5)):
                if difference.getpixel(point) != (0, 0, 0, 0):
                    changed_roof_points.add(point)

    if changed_roof_points != {(32, 5), (37, 5)}:
        raise SystemExit(f"Joint Response Unit roof emitters do not flash independently: {sorted(changed_roof_points)}")
