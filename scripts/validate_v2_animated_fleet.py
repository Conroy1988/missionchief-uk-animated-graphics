...
FIXTURES = json.loads((ROOT / "data/v2.0.0-light-fixtures.json").read_text())["vehicles"]
...
REPORT = ROOT / "data/v2.0.0-animation-qa-report.json"
...
def change_metrics(static: Image.Image, animation_frames: list[Image.Image]) -> dict[str, int]:
    reduced_static = static.resize((100, 100), Image.Resampling.LANCZOS)
    best_pixels = 0
    best_peak = 0
    largest_pixels = 0
    for frame in animation_frames:
        ...
        largest_pixels = max(largest_pixels, sum(max(pixel) >= 18 for pixel in values))
        reduced = frame.resize((100, 100), Image.Resampling.LANCZOS)
        reduced_difference = ImageChops.difference(reduced, reduced_static).convert("RGB")
        reduced_values = list(reduced_difference.get_flattened_data())
        best_pixels = max(best_pixels, sum(max(pixel) >= 18 for pixel in reduced_values))
        best_peak = max(best_peak, max((max(pixel) for pixel in reduced_values), default=0))
    return {
        "changed_pixels_full_max": largest_pixels,
        "changed_pixels_50pct_max": best_pixels,
        "peak_change_50pct": best_peak,
    }
...
def render_sheet(theme: str, frame_index: int, phase: str) -> Path:
    ...
    for index, slot in enumerate(SLOTS):
        ...
        draw.text((x + 5, y + 218), f"{asset_id} / phase {phase}", fill=secondary, font=font)
