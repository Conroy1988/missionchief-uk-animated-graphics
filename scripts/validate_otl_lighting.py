#!/usr/bin/env python3
"""Reject any return of the raised OTL command box or beacon mast."""
...
def require_no_mast_green(
    ...
    if actual:
        raise SystemExit(f"{label} contains saturated green command-mast pixels: {sorted(actual)}")
...
def render_before_after(static: Image.Image, release: str) -> Path:
    ...
    draw.text((24, 18), "OTL roof-mast repair — MissionChief slot 21", font=font(28), fill="white")
    draw.text((340, 64), f"{BASELINE} · raised command box and mast", font=font(17), fill=(255, 166, 166))
    ...
    target = ROOT / "assets" / "previews" / release / "otl-roof-mast-before-after.png"
...
def main() -> None:
    ...
    if master.size != (65, 27):
        raise SystemExit(f"OTL master dimensions changed: {master.size}")
    if static.size != (73, 35):
        raise SystemExit(f"OTL command dimensions changed: {static.size}")
    if old_static.size != (73, 39):
        raise SystemExit(f"OTL baseline dimensions changed: {old_static.size}")
    if master.getchannel("A").getbbox() != (0, 1, 65, 27):
        raise SystemExit("OTL master no longer has the approved low roofline and bottom anchor")
    if static.getchannel("A").getbbox()[-1] != static.height:
        raise SystemExit("OTL command graphic moved away from its bottom-centre map anchor")

    require_isolated_pair(master, (20, 0, 41, 4), {(27, 1), (30, 1)}, "master")
    require_isolated_pair(static, (24, 3, 45, 8), {(31, 5), (34, 5)}, "command static")
    require_no_mast_green(master, (20, 0, 45, 10), "master")
    require_no_mast_green(static, (24, 0, 49, 14), "command static")

    expected_pixels = [(31, 5), (34, 5), (67, 19), (7, 16)]
    actual_pixels = [(int(item["x"]), int(item["y"])) for item in detail["response_light_pixels"]]
    if actual_pixels != expected_pixels:
        raise SystemExit(f"OTL animation fixtures changed: expected {expected_pixels}, found {actual_pixels}")
    if len(fixtures["vehicles"].get(ASSET_ID, [])) != 4:
        raise SystemExit("OTL must retain two roof emitters plus front and rear emitters")
    ...
    with Image.open(ANIMATED_PATH) as animation:
        if int(getattr(animation, "n_frames", 1)) != 12:
            raise SystemExit("OTL APNG must contain 12 full frames")
        for index in range(animation.n_frames):
            animation.seek(index)
            frame = animation.convert("RGBA")
            if index == 0 and ImageChops.difference(frame, static).getbbox() is not None:
                raise SystemExit("OTL APNG frame zero no longer matches the static PNG")
            require_no_unexpected_roof_blue(frame, (24, 3, 45, 8), {(31, 5), (34, 5)}, f"frame {index}")
            require_no_mast_green(frame, (24, 0, 49, 14), f"frame {index}")
            difference = ImageChops.difference(frame, static)
            for point in ((31, 5), (34, 5)):
                if difference.getpixel(point) != (0, 0, 0, 0):
                    changed_roof_points.add(point)

    if changed_roof_points != {(31, 5), (34, 5)}:
        raise SystemExit(f"OTL roof emitters do not flash independently: {sorted(changed_roof_points)}")
    ...
    print(json.dumps({
        ...
        "saturated_green_command_mast_pixels": 0,
