#!/usr/bin/env python3
"""Reject any return of the IRV roof-light blob regression."""
...
ASSET_ID = "police-incident-response-vehicle"
...
def peak_frame(path: Path, static: Image.Image, tagged: bool = False) -> Image.Image:
    ...
    with Image.open(io.BytesIO(data)) as animation:
        for index in range(int(getattr(animation, "n_frames", 1))):
            ...
            decoded.append(animation.convert("RGBA").copy())
    return max(
        decoded,
...
def render_before_after(static: Image.Image) -> Path:
    ...
    draw.text((24, 18), "IRV roof-light repair — MissionChief slot 9", font=font(28), fill="white")
    draw.text((340, 64), f"{BASELINE} · rejected blue modules", font=font(17), fill=(255, 166, 166))
    draw.text((840, 64), "v1.4.4 · isolated roof lenses", font=font(17), fill=(148, 235, 190))
    ...
    target = ROOT / "assets" / "previews" / "v1.4.4" / "irv-roof-light-before-after.png"
...
def main() -> None:
    ...
    if master.size != (64, 22):
        raise SystemExit(f"IRV master dimensions changed: {master.size}")
    if static.size != (72, 30):
        raise SystemExit(f"IRV command dimensions changed: {static.size}")

    require_isolated_pair(master, (24, 0, 39, 4), {(28, 1), (33, 1)}, "master")
    require_isolated_pair(static, (28, 3, 42, 7), {(32, 5), (37, 5)}, "command static")

    expected_pixels = [(32, 5), (37, 5), (64, 20)]
    actual_pixels = [
        (int(item["x"]), int(item["y"]))
        for item in detail["response_light_pixels"]
    ]
    if actual_pixels != expected_pixels:
        raise SystemExit(f"IRV animation fixtures changed: expected {expected_pixels}, found {actual_pixels}")
    if len(fixtures["vehicles"].get(ASSET_ID, [])) != 3:
        raise SystemExit("IRV must retain two roof emitters and one front emitter")
    ...
    with Image.open(ANIMATED_PATH) as animation:
        if int(getattr(animation, "n_frames", 1)) != 12:
            raise SystemExit("IRV APNG must contain 12 full frames")
        for index in range(animation.n_frames):
            animation.seek(index)
            frame = animation.convert("RGBA")
            if index == 0 and ImageChops.difference(frame, static).getbbox() is not None:
                raise SystemExit("IRV APNG frame zero no longer matches the static PNG")
            require_isolated_pair(frame, (28, 3, 42, 7), {(32, 5), (37, 5)}, f"frame {index}")
    ...
    if changed_roof_points != {(32, 5), (37, 5)}:
        raise SystemExit(f"IRV roof emitters do not flash independently: {sorted(changed_roof_points)}")

    preview = render_before_after(static)
