#!/usr/bin/env python3
"""Reject any return of the raised Community First Responder medical cross-box."""
...
ASSET_ID = "community-first-responder"
...
def strong_green(pixel: tuple[int, int, int, int]) -> bool:
    red, green, blue, alpha = pixel
    return alpha >= 128 and green >= 150 and green - red >= 60 and green - blue >= 20


def green_points(image: Image.Image, box: tuple[int, int, int, int]) -> set[tuple[int, int]]:
    rgba = image.convert("RGBA")
    left, top, right, bottom = box
    return {
        (x, y)
        for y in range(top, bottom)
        for x in range(left, right)
        if strong_green(rgba.getpixel((x, y)))
    }
...
def require_no_cross_green(
    image: Image.Image,
    box: tuple[int, int, int, int],
    label: str,
) -> None:
    actual = green_points(image, box)
    if actual:
        raise SystemExit(f"{label} contains saturated green medical cross-box pixels: {sorted(actual)}")
...
def render_before_after(static: Image.Image, release: str) -> Path:
    ...
    draw.text((24, 18), "Community First Responder roof-cross repair — MissionChief slot 23", font=font(28), fill="white")
    draw.text((340, 64), f"{BASELINE} · raised green medical cross-box", font=font(17), fill=(255, 166, 166))
    ...
    target = ROOT / "assets" / "previews" / release / "community-first-responder-roof-cross-before-after.png"
...
def main() -> None:
    ...
    if master.size != (57, 23):
        raise SystemExit(f"Community First Responder master dimensions changed: {master.size}")
    if static.size != (65, 31):
        raise SystemExit(f"Community First Responder command dimensions changed: {static.size}")
    if old_static.size != (65, 33):
        raise SystemExit(f"Community First Responder baseline dimensions changed: {old_static.size}")
    if master.getchannel("A").getbbox() != (0, 1, 57, 23):
        raise SystemExit("Community First Responder master no longer has the approved low roofline and bottom anchor")
    if static.getchannel("A").getbbox()[-1] != static.height:
        raise SystemExit("Community First Responder command graphic moved away from its bottom-centre map anchor")

    require_isolated_pair(master, (16, 0, 34, 5), {(21, 1), (24, 1)}, "master")
    require_isolated_pair(static, (20, 3, 38, 8), {(25, 5), (28, 5)}, "command static")
    require_no_cross_green(master, (14, 0, 36, 8), "master")
    require_no_cross_green(static, (18, 0, 40, 12), "command static")

    expected_pixels = [(25, 5), (28, 5), (59, 16), (7, 15)]
    actual_pixels = [(int(item["x"]), int(item["y"])) for item in detail["response_light_pixels"]]
    if actual_pixels != expected_pixels:
        raise SystemExit(f"Community First Responder animation fixtures changed: expected {expected_pixels}, found {actual_pixels}")
    if len(fixtures["vehicles"].get(ASSET_ID, [])) != 4:
        raise SystemExit("Community First Responder must retain two roof emitters plus front and rear emitters")
    ...
    with Image.open(ANIMATED_PATH) as animation:
        if int(getattr(animation, "n_frames", 1)) != 12:
            raise SystemExit("Community First Responder APNG must contain 12 full frames")
        for index in range(animation.n_frames):
            animation.seek(index)
            frame = animation.convert("RGBA")
            if index == 0 and ImageChops.difference(frame, static).getbbox() is not None:
                raise SystemExit("Community First Responder APNG frame zero no longer matches the static PNG")
            require_no_unexpected_roof_blue(frame, (20, 3, 38, 8), {(25, 5), (28, 5)}, f"frame {index}")
            require_no_cross_green(frame, (18, 0, 40, 12), f"frame {index}")
            difference = ImageChops.difference(frame, static)
            for point in ((25, 5), (28, 5)):
                if difference.getpixel(point) != (0, 0, 0, 0):
                    changed_roof_points.add(point)

    if changed_roof_points != {(25, 5), (28, 5)}:
        raise SystemExit(f"Community First Responder roof emitters do not flash independently: {sorted(changed_roof_points)}")
    ...
    print(json.dumps({
        ...
        "saturated_green_medical_cross_pixels": 0,
