...
14. `python scripts/validate_light_placement.py --report data/v1.4.8-light-placement-report.json`
15. `python scripts/validate_full_fleet_lighting.py`
16. `python scripts/validate_release_scope.py`
17. `python scripts/build_numbered_upload_package.py --version v1.4.8 --profile command`
...
- Master dimensions: 66×25
- Command dimensions: 74×33
- Master alpha bounds: `(0, 1, 66, 25)`
- Static master roof lenses: exactly `(30, 1)` and `(36, 1)`
- Static command roof lenses: exactly `(34, 5)` and `(40, 5)`
- Maximum connected saturated-blue component in the roof housing: one pixel
- No other saturated-blue pixel anywhere in the audited roof region
- Animated response fixtures: `(34, 5)`, `(40, 5)`, `(66, 16)` and `(8, 16)`
...
The release is deployable only when every gate passes and `data/v1.4.8-scope.json` reports no missing or unexpected export path.
